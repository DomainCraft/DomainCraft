...
| `when` | no | Condition: `hasSeed`, `hasEnums`, `hasOwnerTokens`, `hasAuth`, `hasMigration`, `hasMockData`, `hasAddon:dapr`, `notHasAddon:dapr` |
...
| `IsFeatureField` | (IRField method) check if a field is auto-generated by features | `{{ if .IsFeatureField }}` |
...
| `literalValue` | Render a scalar value/default as a literal or parse expression | `{{ literalValue .DatabaseType .DefaultValue }}` |
| `literalDefault` | Render the value-less default (or empty array) | `{{ literalDefault .DatabaseType }}` |
| `literalMember` | Render an enum default as a compile-time member | `{{ literalMember .DatabaseType .DefaultValue }}` |
| `arrayLiteralOpen` / `arrayLiteralClose` | Wrap array elements in the bridge's array syntax | `{{ arrayLiteralOpen .DatabaseType }} … {{ arrayLiteralClose }}` |
| `IsUuid` / `IsText` / `IsString` / `IsInteger` / `IsBigInt` / `IsFloat` / `IsDecimal` / `IsNumeric` / `IsBoolean` / `IsDate` / `IsDateTime` / `IsJson` / `IsJsonB` / `IsEnum` | (IRField methods) type predicates — prefer over `eq .DatabaseType "…"` | `{{ if .IsNumeric }}` |
