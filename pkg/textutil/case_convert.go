...
// SnakeCase and KebabCase convert CamelCase / snake_case / kebab-case /
// space-separated identifiers to snake_case and kebab-case respectively,
// handling acronym runs ("HTTPStatus" → "http_status"), digit groups
// ("HTTP20xOK" → "http_20x_ok"), Unicode letters and CJK non-word
// characters.
//
// Note SnakeCase deliberately differs from ToDatabaseColumnName on
// acronym+digit runs ("IPv4Address" → "i_pv4_address" here vs
// "i_pv4address" for DB columns); each serves its own purpose and neither
// should be used for the other's job.
...
// SnakeCase converts a CamelCase / separated identifier to snake_case.
// Examples: "FirstName" → "first_name", "HTTPServer" → "http_server",
// "HTTP20xOK" → "http_20x_ok", "IPv4Address" → "i_pv4_address".
func SnakeCase(str string) string {
	return camelCaseToLowerCase(str, '_')
}

// KebabCase converts a CamelCase / separated identifier to kebab-case.
// Examples: "FirstName" → "first-name", "HTTPServer" → "http-server",
...
// isAlphabet reports whether r is a letter but not a CJK character.
