...
funcMap["fkName"] = func(name string) string {
	...
	funcMap["isFeatureField"] = func(name string) bool {
		_, ok := specmeta.FeatureFieldNames[name]
		return ok
	}
	...
	if err == nil {
		...
		bridgeFuncs["languageType"] = func(goType string, nullable bool) string {
			...
			if mapped, ok := mapping.Types[goType]; ok {
				if nullable && !specmeta.IsArrayType(goType) {
					return wrapNullable(mapped)
				}
				return mapped
			}

			inner := specmeta.ParseArrayInner(goType)
			...
			if !specmeta.IsPrimitive(strings.ToLower(inner)) {
				...
				if specmeta.IsArrayType(goType) {
		...
		bridgeFuncs["deleteBehaviorName"] = func(value string) string {
			...
			return textutil.PascalCase(strings.ReplaceAll(strings.ReplaceAll(value, "_", " "), "-", " "))
		}
...
func (r *Renderer) Render(project *ir.IRProject, outputDir string) ([]string, error) {
	...
	for _, spec := range r.config.Templates {
		...
		contexts, err := r.renderContexts(spec.For, project)
		if err != nil {
			return nil, err
		}
...
func (r *Renderer) shouldRenderContext(spec TemplateSpec, context RenderContext) bool {
	...
	case "hasOwnerTokens":
		...
		if context.Project != nil {
			for _, e := range context.Project.Entities {
				if e.Permissions != nil {
					for _, roles := range [][]string{e.Permissions.Read, e.Permissions.Create, e.Permissions.Update, e.Permissions.Delete} {
						for _, r := range roles {
							if strings.HasPrefix(r, "@") {
								return true
							}
