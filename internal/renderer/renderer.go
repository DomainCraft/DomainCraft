	...
	if err == nil {
		...
		languageType := func(dbType string, nullable bool) string {
			...
			if specmeta.IsArrayType(dbType) {
				...
				r.log.Warn("type_mappings.yaml has no type for %q; emitting raw type name %q", inner, inner)
				return wrapArray(inner)
			}

			r.log.Warn("type_mappings.yaml has no type for %q; emitting raw type name %q", dbType, dbType)
			return dbType
		}
...
func (r *Renderer) shouldRender(spec TemplateSpec, project *ir.IRProject, packages map[string]string) bool {
	...
	return r.shouldRenderContext(spec, RenderContext{Project: project, Bridge: &r.config, Packages: packages, Migration: r.migration, SeedData: r.seedData, MockData: r.mockData})
}
...
func (r *Renderer) renderContexts(scope string, project *ir.IRProject, pkgs map[string]string) ([]RenderContext, error) {
	...
	case "", "entity":
		...
		for i := range project.Entities {
			contexts = append(contexts, RenderContext{Project: project, Entity: &project.Entities[i], Bridge: &r.config, Packages: pkgs, Migration: r.migration, SeedData: r.seedData, MockData: r.mockData})
		}
		return contexts, nil
	case "project":
		return []RenderContext{{Project: project, Bridge: &r.config, Packages: pkgs, Migration: r.migration, SeedData: r.seedData, MockData: r.mockData}}, nil
	default:
